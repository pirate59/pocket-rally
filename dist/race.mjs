...
export const COURSES = [
 {name:'Desktop Dash',revision:2,tag:'THE DESKTOP GRAND CIRCUIT',desc:'Thread the stationery chicanes.<br>Two jumps. One book-stack flyover.',tags:['TWIN JUMPS','DESK CHICANES','BOOK FLYOVER'],type:'car',color:0xc49360,road:0x303d43,width:6.4,bounds:[44,31],walls:true,gap:[.435,.451],rampStart:.410,extraJumps:[{rampStart:.795,gap:[.818,.834]}],points:[[-32,0,-19],[-20,0,-22],[-10,1,-15],[0,6,0],[12,2,16],[24,0,23],[37,0,18],[36,0,7],[25,0,4],[24,0,-5],[36,0,-10],[34,0,-23],[22,0,-25],[12,0,-17],[0,0,0],[-12,0,17],[-23,0,24],[-36,0,20],[-38,0,8],[-28,0,4],[-25,0,-6],[-37,0,-9]]},
...
COURSES.forEach(course=>{course.revision=(course.revision||0)+1});
...
export function makeTrack(course){
 ...
 for(let i=0;i<=count;i++){let f=i/count*p.length,k=Math.floor(f),t=f-k;raw.push([0,1,2].map(a=>cat(p[wrap(k-1,p.length)][a],p[wrap(k,p.length)][a],p[wrap(k+1,p.length)][a],p[wrap(k+2,p.length)][a],t)));}
 ...
 const barriers=[];
 if(course.walls)for(let i=0;i<n;i+=3){let a=nodes[i],b=nodes[(i+3)%n];if([0,1,2,3].some(k=>nodes[(i+k)%n].gap))continue;if(course.intersection&&Math.hypot(a.x,a.z)<(course.intersection.radius||course.width*.88))continue;for(let side of[-1,1]){const edge=course.width/2+.18;barriers.push({index:i,ax:a.x+a.tz*edge*side,az:a.z-a.tx*edge*side,ay:a.y,bx:b.x+b.tz*edge*side,bz:b.z-b.tx*edge*side,by:b.y,height:.62,width:.34});}}
...
export class Race{
 constructor(track,difficulty='medium'){this.difficulty=DIFFICULTIES[difficulty]?difficulty:'medium';this.track=track;this.time=0;this.countdown=3.4;this.finished=[];this.obstacles=[];this.paused=false;this.boostPickups=makeBoostPickups(track);this.boostPads=makeBoostPads(track);this.cars=COLORS.map((color,i)=>{const slot=i===0?3:i-1;let progress=-8-slot*5,index=wrap(progress,track.n),p=track.nodes[index],lane=(slot%2?1:-1)*1.25;return{id:i,color,name:NAMES[i],x:p.x+p.tz*lane,z:p.z-p.tx*lane,y:p.y,heading:p.heading,vx:0,vz:0,vy:0,speed:0,progress,index,lastSafe:progress,boost:1,boostLocked:false,boostLap:0,catchup:1,padBoostTimer:0,padBoostCooldown:0,airborne:false,boosting:false,drifting:false,offTime:0,stuck:0,respawns:0,finish:null,flash:0};});}
 ...
 step(dt,input={}){
  ...
  for(const c of this.cars){if(c.finish!==null)continue;const tr=this.track,co=tr.course;let near=nearest(tr,c.x,c.z,c.y,c.index),p=near.p,onRoad=near.d<co.width/2,throttle=0,steer=0,brake=false,boost=false;
   ...
   const grip=co.type==='boat'?3.3:brake?1.7:8.2,turn=(brake?2.5:1.85)*(co.type==='boat'?.88:1);c.heading+=steer*turn*clamp(Math.abs(c.speed)/5,0,1)*Math.sign(c.speed||1)*dt*(c.airborne?.48:1);
   ...
   for(const o of this.obstacles){if(Math.abs(c.y-o.y)>2)continue;let dx=c.x-o.x,dz=c.z-o.z,dist=Math.hypot(dx,dz),r=o.r+.72;if(dist<r&&dist>.001){let nx=dx/dist,nz=dz/dist;c.x+=nx*(r-dist);c.z+=nz*(r-dist);let dot=c.vx*nx+c.vz*nz;if(dot<0){c.vx-=nx*dot*1.4;c.vz-=nz*dot*1.4;}}}
   for(const wall of (tr.barrierCells.get(Math.floor(c.x/8)+','+Math.floor(c.z/8))||[])){
    const dx=wall.bx-wall.ax,dz=wall.bz-wall.az,len2=dx*dx+dz*dz,t=clamp(((c.x-wall.ax)*dx+(c.z-wall.az)*dz)/len2,0,1),floor=wall.ay+(wall.by-wall.ay)*t;
    if(c.y>floor+wall.height||c.y+.85<floor)continue;
    const qx=c.x-(wall.ax+dx*t),qz=c.z-(wall.az+dz*t),d=Math.hypot(qx,qz),r=.62+wall.width/2;
    if(d<r&&d>.001){const nx=qx/d,nz=qz/d;c.x+=nx*(r-d);c.z+=nz*(r-d);const impact=c.vx*nx+c.vz*nz;if(impact<0){c.vx-=nx*impact*1.15;c.vz-=nz*impact*1.15;}}
   }
   if(c.progress>=tr.n*3){c.finish=this.time;this.finished.push(c.id);}
  }
  for(let i=0;i<4;i++)for(let j=i+1;j<4;j++){let a=this.cars[i],b=this.cars[j];if(a.finish||b.finish||Math.abs(a.y-b.y)>1.3)continue;let dx=a.x-b.x,dz=a.z-b.z,d=Math.hypot(dx,dz);if(d<1.5&&d>.001){let nx=dx/d,nz=dz/d,push=(1.5-d)*.5;a.x+=nx*push;a.z+=nz*push;b.x-=nx*push;b.z-=nz*push;let rel=(a.vx-b.vx)*nx+(a.vz-b.vz)*nz;if(rel<0){a.vx-=nx*rel*.7;a.vz-=nz*rel*.7;b.vx+=nx*rel*.7;b.vz+=nz*rel*.7;}}}
