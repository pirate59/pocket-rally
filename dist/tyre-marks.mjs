...
import {nearestCarpetRoad} from './carpet-run.mjs';
...
export function tyreMarkStrength(c,previous,dt){
 ...
 const braking=c.braking&&acceleration<-4*engine?clamp(-acceleration/(32*engine),.25,.72):0;
...
export function tyreRoadHeight(track,c,x,z){
 ...
 if(track.course.mode==='collect')return nearestCarpetRoad(track,x,z).d<track.course.width/2-.16?.185:null;
...
export class TyreMarks{
 constructor(world,track,capacity=6000){
  ...
  geometry.setAttribute('position',this.position);geometry.setAttribute('born',this.born);geometry.setAttribute('strength',this.strength);geometry.setAttribute('edge',new THREE.BufferAttribute(edge,1));geometry.setIndex(new THREE.BufferAttribute(indices,1));geometry.setDrawRange(0,0);
  const material=new THREE.ShaderMaterial({transparent:true,depthWrite:false,side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1,
   ...
   vertexShader:`attribute float born;attribute float strength;attribute float edge;uniform float time;varying float opacity;varying float tyreEdge;
    void main(){opacity=strength*(1.0-smoothstep(65.0,100.0,time-born));tyreEdge=edge;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,
   fragmentShader:`varying float opacity;varying float tyreEdge;void main(){float a=opacity*(1.0-smoothstep(.68,1.0,abs(tyreEdge)));if(a<.01)discard;gl_FragColor=vec4(.025,.030,.028,a);}`});
 ...
 wheel(c,side){
  ...
  const x=c.x+t*side*.84*sx-s*.94*sz,z=c.z-s*side*.84*sx-t*.94*sz,y=tyreRoadHeight(this.track,c,x,z);
  return y===null?null:{x,y,z,dx:t*.125*sx,dz:-s*.125*sx};
 }
 segment(a,b,strength,time){
  ...
  this.born.array.fill(time,i*4,i*4+4);this.strength.array.fill(strength,i*4,i*4+4);
 ...
 update(cars,time){
  ...
  for(const c of cars){
   ...
   const strength=tyreMarkStrength(c,prev,dt),distance=prev?Math.hypot(c.x-prev.x,c.z-prev.z):0;
   ...
   for(let step=1;step<=steps;step++){
    ...
    for(let side=0;side<2;side++)if(wheels?.[side]&&next[side]&&Math.hypot(next[side].x-wheels[side].x,next[side].z-wheels[side].z)>.015)this.segment(wheels[side],next[side],strength,time);
    wheels=next;
   }
   now.wheels=wheels;
  }
  if(this.dirty){this.position.needsUpdate=true;this.born.needsUpdate=true;this.strength.needsUpdate=true;this.mesh.geometry.setDrawRange(0,this.count*6);}
 }
}
