...
export function driveRealCar(c,{throttle,steer,brake,onRoad,p,track},dt){
 const speed=Math.hypot(c.vx,c.vz),fx=Math.sin(c.heading),fz=Math.cos(c.heading);
 const forward=c.vx*fx+c.vz*fz,lateral=c.vx*fz-c.vz*fx;
 c.steerAngle=(c.steerAngle||0)+(steer*.48/(1+speed*.025)-(c.steerAngle||0))*(1-Math.exp(-dt*(steer?7:12)));
 ...
 let drive=opposing?throttle*23:throttle*12;
 const catchup=c.catchup||1,target=(c.boosting?29:20)*catchup;
 if(!opposing&&throttle>0){drive*=catchup*clamp((target-forward)/5,0,1);if(c.boosting)drive+=12*clamp((target-forward)/5,0,1);}
 if(!opposing&&throttle<0)drive*=clamp((6+forward)/3,0,1)*.6;
 if(handbrake)drive-=Math.sign(forward)*17;
 drive=clamp(drive,-grip*.9,grip*.9);
 // Braking and acceleration consume some of the same grip used for cornering.
 const lateralBudget=Math.sqrt(Math.max(0,grip*grip-drive*drive))*(handbrake?.42:1);
 const sideForce=clamp(-lateral*(handbrake?2.3:onRoad?14:7),-lateralBudget,lateralBudget);
 const mechanicalYaw=forward/2.65*Math.tan(c.steerAngle)*(handbrake?1.35:1);
 ...
 const yawLimit=(handbrake?grip*1.35:lateralBudget*.92)/Math.max(4,speed);
 ...
 const drag=.10+speed*.010+(onRoad?0:(track.course.offroadDrag??3)*.22);
