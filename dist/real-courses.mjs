...
const piecewiseRunoff=(stops)=> (u)=>{u=(u%1+1)%1;for(let i=0;i<stops.length-1;i++){const a=stops[i],b=stops[i+1];if(u>=a[0]&&u<=b[0]){const t=(u-a[0])/(b[0]-a[0]);return a[1]+(b[1]-a[1])*(t*t*(3-2*t));}}return stops.at(-1)[1]};
const panoramaRunoff=piecewiseRunoff([[0,.72],[.025,1.8],[.08,1.9],[.15,1.45],[.19,.35],[.64,.28],[.72,1.8],[.84,2.05],[.9,1.35],[.96,.78],[1,.72]]);
export const PANORAMA={name:'Mount Panorama',group:3,realWorld:true,revision:2,tag:'REAL WORLD · BATHURST, AUSTRALIA',desc:'Climb the mountain. Thread the Esses.<br>Descend Conrod Straight to the Chase.',tags:['COUNTER-CLOCKWISE','MOUNTAIN TERRAIN','6.213 KM CIRCUIT'],type:'car',color:0x71805a,road:0x343b40,width:6.2,mapScale:3.5,mapSize:[274,414],bounds:[135,204],barrierOffset:.52,barrierOffsetAt:(u)=>panoramaRunoff(u),offroadDrag:3.25,walls:true,jumpAnchors:[],points:route.map(([x,z,h])=>{const p=panoramaPoint(x,z);return[p.x,h*PANORAMA_ELEVATION_SCALE,p.z]})};
