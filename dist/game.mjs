...
let world=new THREE.Group();scene.add(world);let selected=0,track,race,mode='menu',cameraMode='close',cpuDifficulty='medium',cameraHeight=24,carModels=[],colliders=[],animated=[],particles=[],skids=[],soundEnabled=false,audioCtx,engineOsc,engineGain,lastTime=0,accumulator=0,menuClock=0,lastBeep=4,lastFinish=false,shownMessage='',messageUntil=0;
let grandScenery=null,carpetWorld=null,lastCollected=0,mapWasRunning=false;
const visualStyle=new VisualStyle();let courseLights=null,boostWorld=null,lastBoostRefills=0;
const isCollection=()=>COURSES[selected]?.mode==='collect';
const createRace=()=>isCollection()?new CarpetRun(track,cpuDifficulty):new Race(track,cpuDifficulty);
...
function makeVehicle(color,type){const g=new THREE.Group();let wheels=[];const dark=0x203d42,chrome=material(0xbac4c0,.25,.8);
 if(type==='boat'){
  ...
  for(let x of[-.52,.52]){let light=box(.35,.15,.08,new THREE.MeshStandardMaterial({color:0xfff6d4,emissive:0xffeeb0,emissiveIntensity:.45}),x,.76,1.41,g);box(.3,.11,.07,0xb12e29,x,.73,-1.4,g);box(.08,.34,.08,0x2a352e,x,1.04,-1.16,g)}
 ...
 const shadow=mesh(new THREE.CircleGeometry(1.25,24),new THREE.MeshBasicMaterial({color:0x142719,transparent:true,opacity:.16,depthWrite:false}),0,.04,0,g);shadow.rotation.x=-Math.PI/2;shadow.castShadow=false;g.userData={wheels,shadow};return g;
}
function makeTrafficVehicle(color){
 ...
 for(const x of[-.5,.5]){box(.35,.17,.08,0xffedb9,x,.65,1.42,g);box(.3,.15,.08,0xc44f47,x,.65,-1.42,g)}
...
function buildBubbleEdges(barriers){
 const random=rng(8241),count=barriers.length*6,dummy=new THREE.Object3D();
 ...
 let index=0;for(const wall of barriers)for(let j=0;j<6;j++){
  const t=(j+.5)/6,r=.34+random()*.20,x=wall.ax+(wall.bx-wall.ax)*t,z=wall.az+(wall.bz-wall.az)*t,y=wall.ay+(wall.by-wall.ay)*t+r*.88;
...
function buildRoad(){
 ...
 function vertex(p,side,y=0){return[p.x+p.tz*side,p.y+y,p.z-p.tx*side]}
 ...
 if(tr.barriers.length&&co.type!=='boat'){
  const body=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),material(0xffffff),tr.barriers.length),caps=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),material(0xffffff),tr.barriers.length),dummy=new THREE.Object3D();
  tr.barriers.forEach((w,i)=>{const delta=new THREE.Vector3(w.bx-w.ax,w.by-w.ay,w.bz-w.az);dummy.position.set((w.ax+w.bx)/2,(w.ay+w.by)/2+w.height/2,(w.az+w.bz)/2);dummy.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,1),delta.clone().normalize());dummy.scale.set(w.width,w.height,delta.length()+.035);dummy.updateMatrix();body.setMatrixAt(i,dummy.matrix);body.setColorAt(i,new THREE.Color(co.type==='boat'?(Math.floor(w.index/6)%2?0xf08d58:0xe7e9cc):co.type==='buggy'?(Math.floor(w.index/6)%2?0x9b754a:0x5a412a):(Math.floor(w.index/6)%2?0xe9bd51:0x8c5839)));dummy.position.y+=w.height/2+.015;dummy.scale.y=.065;dummy.scale.x=w.width+.08;dummy.updateMatrix();caps.setMatrixAt(i,dummy.matrix);caps.setColorAt(i,new THREE.Color(co.type==='boat'?0xeaf9df:co.type==='buggy'?0xc8a775:(Math.floor(w.index/6)%2?0xffe29a:0xfff4d2)));});
  body.castShadow=true;body.receiveShadow=true;caps.castShadow=true;world.add(body,caps);
 }
...
function clearWorld(){visualStyle.clear();boostWorld?.dispose();boostWorld=null;courseLights?.dispose();courseLights=null;grandScenery?.dispose();grandScenery=null;carpetWorld=null;world.traverse(o=>{if(o.geometry)o.geometry.dispose();if(o.material&&!Array.isArray(o.material)&&o.material.map){o.material.map.dispose();o.material.dispose();}});scene.remove(world);world=new THREE.Group();scene.add(world);carModels=[];animated=[];colliders=[];particles=[];skids=[];}
function setupCourse(index){sun.position.set(-28,65,28);sun.target.position.set(0,0,0);sun.shadow.camera.left=-46;sun.shadow.camera.right=46;sun.shadow.camera.top=39;sun.shadow.camera.bottom=-39;sun.shadow.camera.far=140;sun.shadow.camera.updateProjectionMatrix();selected=index;clearWorld();track=isCollection()?makeCarpetTrack(COURSES[index]):makeTrack(COURSES[index]);race=createRace();const theme=index%3,random=rng(713+index*201);scene.background=new THREE.Color(0x000000);scene.fog=new THREE.Fog(0x000000,115,210);
 if(isCollection()){carpetWorld=buildCarpetWorld({world,track,addCollider});grandScenery=carpetWorld.fader;}else{if(index>=3)buildGrandWorld(random);else if(index===0)buildTable(random);else if(index===1)buildBath(random);else buildGarden(random);buildRoad();}race.obstacles=colliders;
 ...
 document.querySelectorAll('[data-course]').forEach(b=>b.classList.toggle('hidden',Math.floor(Number(b.dataset.course)/3)!==Math.floor(index/3)));document.querySelectorAll('[data-group]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.group)===Math.floor(index/3))));let record=getBest(index);$('best').textContent=record?`BEST ${formatTime(record)}`:'NO RECORD YET';$('loading').classList.add('hidden');configureRaceUI();resize();
}
function getBest(index){try{return Number(localStorage.getItem('pocket-rally-best-'+cpuDifficulty+'-'+index+(COURSES[index].revision?'-v'+COURSES[index].revision:'')))||0}catch{return 0}}
function setVisible(id,visible){$(id).classList.toggle('hidden',!visible)}
function startRace(){$('cpu-difficulty').disabled=true;race=createRace();lastCollected=0;lastBoostRefills=0;closeMap(false);race.obstacles=colliders;mode='race';cameraHeight=raceCameraHeight();viewTarget.set(race.cars[0].x,race.cars[0].y*.7,race.cars[0].z);lastFinish=false;lastBeep=4;shownMessage='';messageUntil=0;accumulator=0;clearKeys();for(const p of particles)world.remove(p.mesh);particles=[];['menu','course-info'].forEach(id=>setVisible(id,false));['hud','driving-hud','minimap','pause'].forEach(id=>setVisible(id,true));document.querySelector('.race-only').classList.remove('hidden');document.body.classList.add('racing');$('results').close();$('pause-dialog').close();if(soundEnabled)initAudio();updateHUD();}
function backToMenu(){closeMap(false);$('cpu-difficulty').disabled=false;mode='menu';race.paused=false;clearKeys();$('pause-dialog').close();$('results').close();['menu','course-info'].forEach(id=>setVisible(id,true));['hud','driving-hud','minimap','pause'].forEach(id=>setVisible(id,false));document.querySelector('.race-only').classList.add('hidden');document.body.classList.remove('racing');$('race-message').textContent='';setupCourse(selected);}
function togglePause(force){if(mode!=='race')return;if($('carpet-map').open)closeMap(false);race.paused=typeof force==='boolean'?force:!race.paused;clearKeys();if(race.paused)$('pause-dialog').showModal();else $('pause-dialog').close();}
function clearKeys(){for(const k in keys)delete keys[k];touchControls.clear()}
function notify(message,duration=1.7){shownMessage=message;messageUntil=performance.now()/1000+duration}
function finish(){lastFinish=true;mode='results';race.paused=true;clearKeys();let me=race.cars[0],ranking=race.ranking(),place=ranking.findIndex(c=>c.id===0)+1,best=getBest(selected);if(me.finish!==null&&(!best||me.finish<best)){try{localStorage.setItem('pocket-rally-best-'+race.difficulty+'-'+selected+(COURSES[selected].revision?'-v'+COURSES[selected].revision:''),String(me.finish))}catch{}}
...
function updateHUD(){if(isCollection()){updateCollectionHUD();return;}let me=race.cars[0],ranking=race.ranking();$('hazard-status').textContent=me.hazard||'';$('position').textContent=ranking.findIndex(c=>c.id===0)+1;$('lap').textContent=Math.min(3,Math.max(1,Math.floor(me.progress/track.n)+1));$('timer').textContent=formatTime(race.time);$('speed').textContent=Math.round(Math.abs(me.speed)*5.1);updateBoostHUD(me);$('standings').innerHTML=ranking.map((c,i)=>`<li class="${c.id===0?'you':''}">${i+1}<span class="car-dot" style="background:${c.color}"></span>${c.name}${c.finish?' ✓':''}</li>`).join('');drawMinimap();}
function drawMinimap(){if(isCollection()){drawCarpetMap($('minimap'),track,race);return;}let cv=$('minimap'),ctx=cv.getContext('2d'),sx=2.35/(track.course.mapScale||1),sz=2.35/(track.course.mapScale||1),cx=120,cz=81;ctx.clearRect(0,0,cv.width,cv.height);ctx.lineWidth=8;ctx.strokeStyle=selected%3===1?'#1676c7':'#66775955';ctx.lineJoin='round';ctx.beginPath();track.nodes.forEach((p,i)=>{i?ctx.lineTo(cx+p.x*sx,cz+p.z*sz):ctx.moveTo(cx+p.x*sx,cz+p.z*sz)});ctx.closePath();ctx.stroke();ctx.lineWidth=2;ctx.strokeStyle='#e9efd9';ctx.stroke();let p=track.nodes[0];ctx.fillStyle='#203b2b';ctx.fillRect(cx+p.x*sx-3,cz+p.z*sz-3,6,6);drawBoostMarkers(ctx,(x,z)=>[cx+x*sx,cz+z*sz],race);for(let c of [...race.cars].reverse()){ctx.beginPath();ctx.arc(cx+c.x*sx,cz+c.z*sz,c.id===0?5:3.8,0,Math.PI*2);ctx.fillStyle=c.color;ctx.fill();ctx.strokeStyle='#fff9e6';ctx.lineWidth=c.id===0?2:1;ctx.stroke();}}
function spawnParticle(c,type){if(particles.length>90)return;const co=COURSES[selected],isWater=co.type==='boat';let color=type==='boost'?0xdfff85:isWater?0xd6fff8:0xa79678;let m=sphere(type==='boost'?.12:.12+Math.random()*.11,new THREE.MeshBasicMaterial({color,transparent:true,opacity:.55,depthWrite:false}),c.x-Math.sin(c.heading)*1.4,c.y+.35,c.z-Math.cos(c.heading)*1.4);m.castShadow=false;particles.push({mesh:m,life:0,max:.5+Math.random()*.4,vx:-c.vx*.15+(Math.random()-.5),vz:-c.vz*.15+(Math.random()-.5),vy:isWater?.6:1.1});}
function updateVisuals(dt,t){
 ...
 for(let i=particles.length-1;i>=0;i--){let p=particles[i];p.life+=dt;if(p.life>p.max){world.remove(p.mesh);p.mesh.geometry.dispose();p.mesh.material.dispose();particles.splice(i,1);continue}p.mesh.position.x+=p.vx*dt;p.mesh.position.z+=p.vz*dt;p.mesh.position.y+=p.vy*dt;p.mesh.material.opacity=.5*(1-p.life/p.max);p.mesh.scale.setScalar(1+p.life*2);}
...
function selectDifficulty(value){cpuDifficulty=['easy','medium','hard'].includes(value)?value:'medium';$('cpu-difficulty').value=cpuDifficulty;try{localStorage.setItem('pocket-rally-cpu',cpuDifficulty)}catch{}setupCourse(selected);}
...
function frame(ms){requestAnimationFrame(frame);let t=ms/1000,dt=Math.min(.05,lastTime?t-lastTime:.016);lastTime=t;if(!race)return;
 if(mode==='race'&&!race.paused){accumulator+=dt;while(accumulator>=1/90){race.step(1/90,{forward:keys.KeyW||keys.ArrowUp||touchControls.keys.KeyW,reverse:keys.KeyS||keys.ArrowDown||touchControls.keys.KeyS,left:keys.KeyA||keys.ArrowLeft||touchControls.keys.KeyA,right:keys.KeyD||keys.ArrowRight||touchControls.keys.KeyD,brake:keys.Space||touchControls.keys.Space,boost:keys.ShiftLeft||keys.ShiftRight||touchControls.keys.ShiftLeft});accumulator-=1/90;}
 ...
 let message=race.countdown>.05?String(Math.min(3,Math.ceil(race.countdown))):race.time<.65?'GO!':t<messageUntil?shownMessage:race.cars[0].airborne&&race.cars[0].y>1.5?'AIR TIME!':'';$('race-message').textContent=message;$('race-message').classList.toggle('small',message.length>3);
 ...
 updateVisuals(dt,t);
 if(mode==='menu'){
  let w=innerWidth,h=innerHeight,aspect=w/h;const scale=track.course.mapScale||1;const target=w<540?new THREE.Vector3(0,0,0):new THREE.Vector3(-13*scale,0,8*scale);viewTarget.lerp(target,1-Math.exp(-dt*4));let height=(w<540?108:Math.max(95,144/aspect))*scale;camera.left=-height*aspect/2;camera.right=height*aspect/2;camera.top=height/2;camera.bottom=-height/2;camera.position.copy(viewTarget).add(offset.clone().multiplyScalar(1.45*scale));camera.lookAt(viewTarget);
  if(w<540)camera.setViewOffset(w,h,0,h*.21,w,h);else camera.clearViewOffset();
 }else if(cameraMode==='chase'){camera.clearViewOffset();updateChaseCamera();viewTarget.set(race.cars[0].x,Math.max(0,race.cars[0].y*.7),race.cars[0].z);
 }else{camera.clearViewOffset();let c=race.cars[0];const lead=cameraMode==='close'?.14:.35;temp.set(c.x+c.vx*lead,Math.max(0,c.y*.7),c.z+c.vz*lead);viewTarget.lerp(temp,1-Math.exp(-dt*(cameraMode==='close'?7.5:4.3)));camera.position.copy(viewTarget).add(offset);camera.lookAt(viewTarget);cameraHeight+=(raceCameraHeight()-cameraHeight)*(1-Math.exp(-dt*6));let height=cameraHeight,aspect=innerWidth/innerHeight;camera.left=-height*aspect/2;camera.right=height*aspect/2;camera.top=height/2;camera.bottom=-height/2;}
 if(track.course.mapScale){const overview=mode==='menu',c=race.cars[0],x=overview?0:c.x,z=overview?0:c.z,r=overview?160:46;sun.position.set(x-28,overview?200:65,z+28);sun.target.position.set(x,0,z);sun.shadow.camera.left=-r;sun.shadow.camera.right=r;sun.shadow.camera.top=r;sun.shadow.camera.bottom=-r;sun.shadow.camera.far=overview?450:140;sun.shadow.camera.updateProjectionMatrix();scene.fog.near=overview?450:115;scene.fog.far=overview?950:210;}camera.updateProjectionMatrix();const activeCamera=mode!=='menu'&&cameraMode==='chase'?chaseCamera:camera;grandScenery?.update(activeCamera,race.cars[0],dt,mode!=='menu');carpetWorld?.update(race,t,activeCamera);courseLights?.update(race.cars,activeCamera,t);visualStyle.update();renderer.render(scene,activeCamera);
}
function configureRaceUI(){
 $('boost-help').textContent=isCollection()?'Boost: shared pickups, pads and catch-up speed':'Boost: '+race.boostPickups.length+' shared pickup'+(race.boostPickups.length>1?'s':'')+' + '+race.boostPads.length+' reusable pads · lap refill';
 const collect=isCollection();document.body.classList.toggle('collection-mode',collect);
 $('minimap').width=collect?207:240;$('minimap').height=collect?299:165;
 ...
 $('cpu-difficulty').previousElementSibling.textContent='CPU';$('cpu-difficulty').setAttribute('aria-label','CPU difficulty');
 ...
 document.querySelector('.intro').innerHTML=collect?'Collect every block before your three rivals.<br>Everyone has their own set. Each block counts once.':'Ordinary places. Extraordinary races.<br>Three rivals. No room for hesitation.';
...
$('sound').onclick=()=>{soundEnabled=!soundEnabled;if(soundEnabled)initAudio();$('sound-state').textContent=soundEnabled?'ON':'OFF';$('sound').setAttribute('aria-label',soundEnabled?'Disable sound':'Enable sound');if(soundEnabled)beep(600)};
...
document.querySelectorAll('[data-group]').forEach(b=>b.addEventListener('click',()=>setupCourse(Number(b.dataset.group)===2?6:Number(b.dataset.group)*3+(selected===6?0:selected%3))));setupCourse(6);requestAnimationFrame(frame);
