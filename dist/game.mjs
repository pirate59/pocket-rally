import {TyreMarks} from './tyre-marks.mjs';
...
let world=new THREE.Group();scene.add(world);let selected=0,track,race,mode='menu',cameraMode='close',cpuDifficulty='medium',selectedEngine='commercial',cameraHeight=24,carModels=[],colliders=[],animated=[],particles=[],tyreMarks=null,soundEnabled=false,audioCtx,engineOsc,engineGain,lastTime=0,accumulator=0,menuClock=0,lastBeep=4,lastFinish=false,shownMessage='',messageUntil=0;
...
const createRace=()=>isCollection()?new CarpetRun(track,cpuDifficulty,undefined,selectedEngine):new Race(track,cpuDifficulty,selectedEngine);
...
function setupCourse(index){sun.position.set(-28,65,28);sun.target.position.set(0,0,0);sun.shadow.camera.left=-46;sun.shadow.camera.right=46;sun.shadow.camera.top=39;sun.shadow.camera.bottom=-39;sun.shadow.camera.far=140;sun.shadow.camera.updateProjectionMatrix();selected=index;clearWorld();track=isCollection()?makeCarpetTrack(COURSES[index]):makeTrack(COURSES[index]);race=createRace();tyreMarks=new TyreMarks(world,track);const theme=index%3,random=rng(713+index*201);scene.background=new THREE.Color(0x000000);scene.fog=new THREE.Fog(0x000000,115,210);
...
function getBest(index){try{return Number(localStorage.getItem('pocket-rally-best-'+cpuDifficulty+'-'+index+(COURSES[index].revision?'-v'+COURSES[index].revision:'')+engineRecordSuffix(selectedEngine)))||0}catch{return 0}}
function setVisible(id,visible){$(id).classList.toggle('hidden',!visible)}
function startRace(){tyreMarks?.clear();$('engine-class').disabled=true;$('cpu-difficulty').disabled=true;race=createRace();lastCollected=0;lastBoostRefills=0;closeMap(false);race.obstacles=colliders;mode='race';cameraHeight=raceCameraHeight();viewTarget.set(race.cars[0].x,race.cars[0].y*.7,race.cars[0].z);lastFinish=false;lastBeep=4;shownMessage='';messageUntil=0;accumulator=0;clearKeys();for(const p of particles)world.remove(p.mesh);particles=[];['menu','course-info'].forEach(id=>setVisible(id,false));['hud','driving-hud','minimap','pause'].forEach(id=>setVisible(id,true));document.querySelector('.race-only').classList.remove('hidden');document.body.classList.add('racing');$('results').close();$('pause-dialog').close();if(soundEnabled)initAudio();updateHUD();}
function backToMenu(){$('engine-class').disabled=false;closeMap(false);$('cpu-difficulty').disabled=false;mode='menu';race.paused=false;clearKeys();$('pause-dialog').close();$('results').close();['menu','course-info'].forEach(id=>setVisible(id,true));['hud','driving-hud','minimap','pause'].forEach(id=>setVisible(id,false));document.querySelector('.race-only').classList.add('hidden');document.body.classList.remove('racing');$('race-message').textContent='';setupCourse(selected);}
...
function finish(){lastFinish=true;mode='results';race.paused=true;clearKeys();let me=race.cars[0],ranking=race.ranking(),place=ranking.findIndex(c=>c.id===0)+1,best=getBest(selected);if(me.finish!==null&&(!best||me.finish<best)){try{localStorage.setItem('pocket-rally-best-'+race.difficulty+'-'+selected+(COURSES[selected].revision?'-v'+COURSES[selected].revision:'')+engineRecordSuffix(race.engineClass),String(me.finish))}catch{}}
