...
function buildRoad(){
 ...
 if(tr.barriers.length&&co.type!=='boat'){
  ...
  tr.barriers.forEach((w,i)=>{const delta=new THREE.Vector3(w.bx-w.ax,w.by-w.ay,w.bz-w.az);dummy.position.set((w.ax+w.bx)/2,(w.ay+w.by)/2+w.height/2,(w.az+w.bz)/2);dummy.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,1),delta.clone().normalize());dummy.scale.set(w.width,w.height,delta.length()+.035);dummy.updateMatrix();body.setMatrixAt(i,dummy.matrix);body.setColorAt(i,new THREE.Color(co.type==='boat'?(Math.floor(w.index/6)%2?0xf08d58:0xe7e9cc):co.type==='buggy'?(Math.floor(w.index/6)%2?0x9b754a:0x5a412a):(co.realWorld?(Math.floor(w.index/18)%2?0xeee9dc:0xb8463d):(Math.floor(w.index/6)%2?0xe9bd51:0x8c5839))));dummy.position.y+=w.height/2+.015;dummy.scale.y=.065;dummy.scale.x=w.width+.08;dummy.updateMatrix();caps.setMatrixAt(i,dummy.matrix);caps.setColorAt(i,new THREE.Color(co.type==='boat'?0xeaf9df:co.type==='buggy'?0xc8a775:(co.realWorld?0xe9e5d8:(Math.floor(w.index/6)%2?0xffe29a:0xfff4d2))));});
...
function setupCourse(index){sun.position.set(-28,65,28);sun.target.position.set(0,0,0);sun.shadow.camera.left=-46;sun.shadow.camera.right=46;sun.shadow.camera.top=39;sun.shadow.camera.bottom=-39;sun.shadow.camera.far=140;sun.shadow.camera.updateProjectionMatrix();selected=index;clearWorld();track=isCollection()?makeCarpetTrack(COURSES[index]):makeTrack(COURSES[index]);race=createRace();const theme=index%3,random=rng(713+index*201);scene.background=new THREE.Color(0x000000);scene.fog=new THREE.Fog(0x000000,115,210);
 if(isCollection()){carpetWorld=buildCarpetWorld({world,track,addCollider});grandScenery=carpetWorld.fader;}else{if(track.course.realWorld)panoramaWorld=buildPanoramaWorld({world,track,addCollider});else if(index>=3)buildGrandWorld(random);else if(index===0)buildTable(random);else if(index===1)buildBath(random);else buildGarden(random);buildRoad();}race.obstacles=colliders;
...
function updateHUD(){if(isCollection()){updateCollectionHUD();return;}let me=race.cars[0],ranking=race.ranking();$('hazard-status').textContent=track.course.realWorld?((track.sections.filter(s=>s.index<=me.index).at(-1)||track.sections[0]).name+' · '+Math.round(Math.max(0,me.y-.13)/.18)+' m ↑'):me.hazard||'';$('position').textContent=ranking.findIndex(c=>c.id===0)+1;$('lap').textContent=Math.min(3,Math.max(1,Math.floor(me.progress/track.n)+1));$('timer').textContent=formatTime(race.time);$('speed').textContent=Math.round(Math.abs(me.speed)*5.1);updateBoostHUD(me);$('standings').innerHTML=ranking.map((c,i)=>`<li class="${c.id===0?'you':''}">${i+1}<span class="car-dot" style="background:${c.color}"></span>${c.name}${c.finish?' ✓':''}</li>`).join('');drawMinimap();}
...
function configureRaceUI(){
 ...
 document.body.classList.toggle('real-world-mode',!!track.course.realWorld);$('minimap').width=collect?207:track.course.realWorld?180:240;$('minimap').height=collect?299:track.course.realWorld?266:165;
 ...
 document.querySelector('.intro').innerHTML=collect?'Collect every block before your three rivals.<br>Everyone has their own set. Each block counts once.':track.course.realWorld?'Famous circuits. Miniature motorsport.<br>Climb, crest and descend the mountain.':'Ordinary places. Extraordinary races.<br>Three rivals. No room for hesitation.';
