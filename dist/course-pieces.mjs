...
export function parsePiece(text){
 ...
 for(const w of words.slice(1)){const m=w.match(/^([a-z]+)=(-?[\d.]+)$/i);if(m)options[m[1]]=Number(m[2]);else if(w==='jump')options.jump=true;}
...
// ---- Tracing ---------------------------------------------------------------
function traceStraight(c,{length,rise=0}){
 ...
 for(let i=0;i<n;i++){const t=i/n;points.push([c.x+f.x*length*t,c.y+rise*ease(t),c.z+f.z*length*t]);}
...
function primitives(piece){
 if(piece.kind==='straight')return[{...piece}];
...
export function describePiece(piece){
 ...
 return piece.kind==='straight'?`straight ${piece.length}${piece.jump?' (jump)':''}`:piece.kind==='turn'?`${piece.angle}° ${side} ${piece.profile}`:`${piece.shape} ${side} hairpin`;
}

// ---- Stage 1: layout -------------------------------------------------------
export function layoutCourse(spec,{start={x:0,z:0,y:0,heading:0}}={}){
 ...
 pieces.forEach((piece,index)=>{
  ...
  for(const prim of primitives(piece)){const traced=prim.kind==='straight'?traceStraight(cursor,prim):traceArc(cursor,prim);points.push(...traced.points);cursor=traced.cursor;pieceLength+=traced.length;}
 ...
 return{pieces:ranges,points,cursor,start,length,jumpAnchors,closure,extent:{minX:Math.min(...xs),maxX:Math.max(...xs),minZ:Math.min(...zs),maxZ:Math.max(...zs)}};
}
...
export function recentreLayout(layout,centre={x:0,z:0}){
 ...
 return{...layout,points:layout.points.map(([x,y,z])=>[x+dx,y,z+dz]),jumpAnchors:layout.jumpAnchors.map(([x,z])=>[x+dx,z+dz]),cursor:move(layout.cursor),start:move(layout.start),pieces:layout.pieces.map(r=>({...r,start:move(r.start),end:move(r.end)})),extent:{minX:e.minX+dx,maxX:e.maxX+dx,minZ:e.minZ+dz,maxZ:e.maxZ+dz}};
}
...
export function validateLayout(layout,{width=6.8,tolerance=.25,headingTolerance=.02}={}){
 ...
 for(const r of pieces)if(Math.abs(r.piece.rise||0)>r.length*.35)problems.push(`Piece ${r.index} (${r.label}): rise of ${r.piece.rise} is too steep for its ${r.length.toFixed(1)} unit length`);
 ...
 const stride=Math.max(1,Math.round(1/STEP)),samples=[];for(let i=0;i<points.length;i+=stride)samples.push({i,x:points[i][0],y:points[i][1],z:points[i][2]});
 ...
 for(let a=0;a<n;a++)for(let b=a+skip;b<n;b++){
  ...
  const p=samples[a],q=samples[b],d=Math.hypot(p.x-q.x,p.z-q.z);if(d>=width+.2)continue;
...
const isHarsh=piece=>piece.kind==='hairpin'||(piece.kind==='turn'&&piece.profile==='sharp'&&piece.angle>=90);
const isFlatStraight=piece=>piece.kind==='straight'&&!(piece.rise||0);
...
export function checkRules(layout){
 ...
 for(const r of jumps){
  ...
  if(next.piece.kind==='straight'&&run<JUMP_LANDING_SAFE)problems.push(`${name(r)}: the landing runs onto a slope at ${name(next)}; keep the ${JUMP_LANDING_SAFE} units after the crest flat`);
...
export function makeCourse(spec,{name='Imagined Circuit',tag='AI-BUILT CIRCUIT',desc='',tags=[],width,theme='table',barrierOffset,start,margin=5,recentre=true,centre={x:0,z:0},grand=false,hazard,hazardLabel,hazardAnchor}={}){
 ...
 let layout=layoutCourse(spec,{start});if(recentre)layout=recentreLayout(layout,centre);const check=validateLayout(layout,{width});
 if(!check.ok)throw new Error('Course layout is not connected up:\n - '+check.problems.join('\n - '));
 const e=layout.extent,halfX=Math.max(Math.abs(e.minX),Math.abs(e.maxX))+margin,halfZ=Math.max(Math.abs(e.minZ),Math.abs(e.maxZ))+margin,span=Math.max(halfX,halfZ)*2;
